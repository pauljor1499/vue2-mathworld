...
export default {
    ...
    data() {
        return {
            get_questions: MW_UpdateAssignment.prototype.getAllQuestions(),
            all_questions: [],
    ...
    methods: {
        ...
        addQuestion(index, item) {
            ...
            MW_UpdateAssignment.prototype.addQuestionType(item);
        },

        getCurrentData() {
            var selected_questions =
                MW_UpdateAssignment.prototype.getSelectedQuestions(
                    this.$route.params.assignment_code
                );

            for (var i = 0; i < selected_questions.length; i++) {
                var index = this.all_questions.findIndex(
                    (item) => item.id === selected_questions[i].id
                );
                this.$emit("add_question", this.all_questions[index]);
                this.$emit("add_current_questions", this.all_questions[index]);
                MW_UpdateAssignment.prototype.addQuestionType(
                    this.all_questions[index]
                );
                this.all_questions.splice(index, 1);
            }
        },
        ...
        remove(item) {
            this.filter.added_teks.splice(
                this.filter.added_teks.indexOf(item),
                1
            );
        },
        ...
        compareValues(arr1, arr2) {
            if (this.filter.added_teks != null) {
    ...
    watch: {
        return_question() {
            this.all_questions.unshift(JSON.parse(this.return_question));
            MW_UpdateAssignment.prototype.removeQuestionType(
                JSON.parse(this.return_question)
            );
        },

        "filter.added_teks"() {
            if (this.filter.added_teks == "") this.filter.added_teks = null;
        },
    },
